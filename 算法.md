...
> 动态规划问题不能脱离**状态和决策**，**时间复杂度=状态×决策** **化零为整（状态表示）， 化整为零（状态迁移）**
...
每种体积是 $v_i$，价值是 $w_i$ 求解一个选物品的方案，物品 **总体积** 不超过背包的 容量，且 **总价值最大**

**思路很简单: 每种物品按照相应的模板解法**

> **注意：多重背包可以被优化成0-1背包一起求解**
...
![](https://vastaitech.feishu.cn/space/api/box/stream/download/asynccode/?code=ZTMyMDVjZTYxY2RlNDlkNzgyZjliMjQyMTNmYWMyMzBfSHB0S295Mks0d0xLMVZ4UnI5R2h2dW1OWUxUWjFydWlfVG9rZW46Ym94Y25CaDZiVE9tUDBpN21IRGZxVlpNck9lXzE2NzgwOTYzOTQ6MTY3ODA5OTk5NF9WNA)
...
![](https://vastaitech.feishu.cn/space/api/box/stream/download/asynccode/?code=MDc5MTUyZTYzOGFiN2ZjMDZhNjg3MzgxZDcyMWM0NjlfNEg4U2VuWGI1WVJtaHJveG1xTXdycU5WZDBSa3cxSHRfVG9rZW46Ym94Y25FSWduNFZHZWdMaEx1V05ER0MyTDZlXzE2NzgwOTYzOTQ6MTY3ODA5OTk5NF9WNA)
...
> 每步同时走 i1 + j1 == i2 + j2 必然成立 可令k = i1 + j1 则$dp[i_1][j_1][i_2][j_2] $可转化为$ dp[k][i_1][i_2]$
...
**算法思路**： 戳破气球可以视作一个区间合并，最终结果要求算出戳破所有气球后获得的最大硬币数量。区间 dp 最重要的是**明白区间**$[l,r]$**代表的含义**（这题应表示为区间$[l,r]$内的所有气球都被戳破），其次是$[l,r]$区间的划分。
...
![](https://vastaitech.feishu.cn/space/api/box/stream/download/asynccode/?code=MTk5OGMwZmZhNzY0MjM0NTBkOTEzY2JmNTZjOTE3YWVfV2F0dHk4Sms3T0Rtd3MxYkdDeHFKZUVHRGxsc2czOWVfVG9rZW46Ym94Y25laXlLUENwbk1mOTBGMElRZUU1RkplXzE2NzgwOTYzOTQ6MTY3ODA5OTk5NF9WNA)
...
![](https://vastaitech.feishu.cn/space/api/box/stream/download/asynccode/?code=N2EwYmNmNzY2ODU3NGUyZmI1YjY1YWQ2ZWQ4YWMzMzFfdnNEV1V1clJxYXVTUWRsOXhmdW4wMjhwSVdObDg0Vm5fVG9rZW46Ym94Y250aG81Wmo3THU3NVd6aFdVN1F3ZktjXzE2NzgwOTYzOTQ6MTY3ODA5OTk5NF9WNA)
...
  将 $n $转换成字符串 $s$，定义 $f(i,isLimit,isNum) $ 表示构造从左往右第 iii 位及其之后数位的合法方案数，其中：

-   $\textit{isLimit} $ 表示当前是否受到了 $n$ 的约束。若为真，则第 iii 位填入的数字至多为 $s[i]$，否则至多为 $9 $。例如 $n=234$，如果前面填了 $23$，那么最后一位至多填 $4$；如果前面填的不是 $23$，那么最后一位至多填 $9$。如果在受到约束的情况下填了$s[i]$，那么后续填入的数字仍会受到 $n$ 的约束。
...
###   [Leetcode902.最大为N的数字组合](https://leetcode.cn/problems/numbers-at-most-n-given-digit-set/description/)

  给定一个按 **非递减顺序** 排列的数字数组 `digits` 。你可以用任意次数 `digits[i]` 来写的数字。例如，如果 `digits = ['1','3','5']`，我们可以写数字，如 `'13'`, `'551'`, 和 `'1351315'`。

  返回 _可以生成的小于或等于给定整数_ _`n`_ _的正整数的个数_ 。
...
**提示1**：将**所有子串**按照其末尾字符的下标分组。 **提示2**：考虑两组**相邻**的子串：以s[i−1]结尾的子串、以s[i]结尾的子串。 **提示3**：以s[i]结尾的子串，可以看成是以s[i−1]结尾的子串，在末尾添加上s[i]组成。
